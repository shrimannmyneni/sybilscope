...
- Home page lists saved analyses with AI-generated summaries
- Click an analysis card to open the force-directed graph
- Or paste wallet addresses in the input box and click **Analyze**
- Graph shows red (confirmed sybil) / yellow (likely) / amber (suspicious) / green (clean) nodes
- Click any cluster or node for AI verdict, evidence, and confidence score
...
Runs the full pipeline and writes `analysis_output.json` to the project root.

### 6. Build the curated demo dataset

```bash
python3 scripts/build_demo_subset.py
```

Produces `demo_curated_cache.json` — 9 confirmed sybils + 15 clean legitimate wallets with zero tx overlap, for a balanced demo with no false positives.
...
```
sybilscope/
  main.jac              — JAC entry point: Wallet/Transfer/Cluster nodes, all walkers,
                          by llm() declarations, agentic investigation loop
  data_fetcher.py       — Etherscan API, clustering (funding + Louvain), all detection
                          signals, build_json_output(), generate_analysis_narrative()
  jac.toml              — JAC project config

server.py               — FastAPI backend: serves frontend, GET /analyses,
                          GET /analysis/{name}, POST /analyze
frontend/
  home.html             — Landing page: saved analyses grid + educational content
  index.html            — D3.js force-directed graph + AI verdict panel

pre_cache.py            — Pre-fetch Etherscan data for demo addresses
scripts/
  build_demo_subset.py  — Builds demo_curated_cache.json: 9 sybils + 15 clean legits
                          with no tx overlap (balanced demo dataset)

data/
  eliminatedSybilAttackers.csv  — 14,196 confirmed HOP Protocol Sybil addresses
  arbiscan_hop_holders.csv      — HOP token holder list (legitimate baseline)
  sybil_overlap.csv             — 82 addresses confirmed sybil AND HOP holders

DEMO_SCRIPT.md          — 5-minute live demo script with Q&A cheat sheet
```
